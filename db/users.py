...
def creat_table(table_style):
    ...
    print(sql)
    mycursor.execute(sql.lower())
    mycursor.close()

#資料表style

#建立use table(id, name, email)
comment_style = "`users`(`userid` BINARY(16) NOT NULL PRIMARY KEY, `email` VARCHAR(50) NOT NULL, `name` VARCHAR(50) NOT NULL, `password` VARCHAR(64) NOT NULL, `time` DATE NOT NULL)"
